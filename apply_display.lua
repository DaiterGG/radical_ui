...
function apply_display.draw_box(x, y, w, h, color_rgba)
  if not color_rgba then return end
  love.graphics.setColor(color_rgba)
  love.graphics.rectangle("fill", x, y, w, h)
end

-- draw a box border (outline). border = { width, radius, color }
function apply_display.draw_box_border(x, y, w, h, border, color_rgba)
  if not border or not color_rgba then return end
...
function apply_display.draw_polygon(x, y, points, color_rgba)
  if not color_rgba or not points then return end
  local verts = {}
...
function apply_display.draw_polyline(x, y, points, width, color_rgba)
  if not color_rgba or not points then return end
  local verts = {}
  for i, p in ipairs(points) do
    verts[#verts + 1] = x + p[1]
...
function apply_display.draw_background(x, y, w, h, bg, border, polyline, opts)
  opts = opts or {}
  ...
  if polyline then
    ...
    if border and border.color then
      apply_display.draw_polyline(x, y, pts, border.width, { border.color:to_rgba() })
    end
    return
  end
