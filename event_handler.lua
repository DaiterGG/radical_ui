...
-- mouse input is fully polled by input_state:poll() (the game's just input can
-- consume the raw mouse events), so this handler only covers what polling
-- can't: wheel (wheelmoved) and keyboard (inputchanged -> modifiers/keybinding).
-- receive() queues every event and process() drains the queue each update;
-- events without a use yet are TODO.
...
-- drain the queue; per-frame deltas were cleared by input_state:poll() (which
-- runs before this), so wheel accumulates cleanly
---@param ctx table
function event_handler.process(ctx)
  ...
  for i = 1, #queue do
    ...
    elseif event.name == "gamepadaxis" or event.name == "gamepadpressed"
        or event.name == "gamepadreleased"
        or event.name == "joystickaxis" or event.name == "joystickpressed"
        or event.name == "joystickreleased" then
